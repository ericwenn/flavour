...
class Flavour_WordPress_Framework_Templates extends Flavour_WordPress_Framework_Initialize {
	...
	*/
	...
	/**
	* Prints the post's first embedded video markup
	* @author Eric Wennerberg
	* @since 0.2.0
	* @version 0.2.0
	...
	*/
	function the_embedded_video() {
		if($this->get_the_embedded_video() !== false) {
			echo $this->get_the_embedded_video();
			return true;
		} else {
			return false;
		}
	...
	* @since 0.2.0
	* @version 0.2.2
	* @return str The embedded video || bool false
	*/
	function get_the_embedded_video() {
		$content = apply_filters('the_content', get_the_content());
		$content = str_replace(']]>', ']]&gt;', $content);
		$matches = array();
		if(preg_match('/(<embed.*?<\/embed>|<iframe.*<\/iframe>)/si', $content, $matches)) {
			return '<div class="entry-video featured-image-padding">'.$matches[1].'</div>';
		} else {
			return false;
		}
	}
	...
	/**
	* Prints the post's first embedded audio markup
	* @author Eric Wennerberg
	* @since 0.2.0
	* @version 0.2.2
	* @uses func $this->get_the_embedded_audio
	*/
	function the_embedded_audio() {
		echo $this->get_the_embedded_audio();
	}


	function get_the_embedded_audio() {
		$content = apply_filters('the_content', get_the_content());
		$content = str_replace(']]>', ']]&gt;', $content);
		$matches = array();
		if(preg_match('/(<audio.*?<\/audio>)/si', $content, $matches)) {
			return $matches[1];
		} else {
			return false;
		}
	...
	*/
	...
	/**
	* Returns the attachment id for the most relevant picture of the post
	* @author Eric Wennerberg
	* @since 0.2.3
	* @version 0.2.3
	*/
