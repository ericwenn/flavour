...
class Flavour_WordPress_Framework_ShortCodes extends Flavour_WordPress_Framework_Templates {
	...
	*/
	function shortcodes_tinymce_plugins($plugins) {
		$plugins['flavour_shortcodes'] = get_template_directory_uri().'/Flavour/admin-script-shortcodes.js';
		return $plugins;
	}
	...
	*/
	function shortcodes_accordion_child($atts, $content = null) {
		extract(shortcode_atts(array(
			'title' => ''
		), $atts));

		$output = '<div class="flavour-accordion-section flavour-accordion-section--active">';
		$output .= '<div class="flavour-accordion-section-handle">';
		$output .= '<h4 class="flavour-accordion-section-title">'.$title.'</h4>';
		$output .= '</div>';
		$output .= '<div class="flavour-accordion-section-content">';
		$output .= do_shortcode($content);
		$output .= '</div>';
		$output .= '</div>';
		$output = $this->shortcode_sanitize($output, 'flavour-accordion-section-content', 'div');
		return $output;
	}
	...
	*/
	function shortcode_sanitize($input = '', $class = '', $element = 'div') {
		if($class == '') {
			$pattern = '/(<'.$element.'.*?>) *?<br *?\/*?>/i';
		} else {
			$pattern = '/(<'.$element.'.*?class="'.$class.'".*?>) *?<br *?\/*?>/i';
		}
